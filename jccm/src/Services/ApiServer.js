...
export const setupApiHandlers = () => {
    ...
    ipcMain.handle('saAdoptDevice', async (event, args) => {
        ...
        const { organization, site, address, port, username, password, ...others } = args;
        ...
        try {
            const api = siteId
                ? `orgs/${orgId}/ocdevices/outbound_ssh_cmd?site_id=${siteId}`
                : `orgs/${orgId}/ocdevices/outbound_ssh_cmd`;
